...
class Settings(BaseSettings):
    ...
    #   share=0.5（默认）→ 一半走 NVDEC 一半走 CPU，12 路 = 6 硬解 + 6 软解
    #   share=0          → 全走 NVDEC，这时 12 路会一起挤 2 个 NVDEC 引擎，
    #                      比 6 路还慢，那就要把这个数压回 6
    # 只改一边是这一串里最容易踩的坑，两次都踩过了。
    #
    # 显存不是约束了：建完一路会把缓存还回去（vision_service 的 gpumem.trim），
    # 实测整卡才用 6.9/31.8 GiB。软解那几路不占显存，占的是内存（每路两三百 MB）。
