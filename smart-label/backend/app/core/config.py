...
class Settings(BaseSettings):
    ...
    # 一棵树里口腔和皮肤都有的素材（狗场合作那批）。多个用逗号分开。
    #
    #   雷喏斯达-狗场合作/雷喏斯达20260912/001比熊小白/口腔牙齿-雷喏斯达20260912-001/*.jpg
    #                     └ 日期          └ 狗         └ 品类：靠名字里的关键字分口腔/皮肤
    #
    # 比 口腔验证/ 多一层。而且**不能按文件夹名里的编号认**——现场那批里
    # 003金毛秃子/皮肤瘙痒-...-002、005金毛年糕/口腔牙齿-...-004 编号都是错的，
    # 按编号分会把照片挂到别的狗名下。只认 口腔|牙齿 / 皮肤|瘙痒 这两组关键字。
    mixed_photo_dirs: str = "雷喏斯达-狗场合作"
    ...
    # **等解码占七成、检测只占一成半**。瓶颈不是 GPU 算力（利用率 77% 时显存带宽
    # 才 3%），是每一路自己那条解码流——一路只能串行解，要快只能多开几路。
    #
    # 2026-09-21 三种配置都量过，每路耗时：
    #   6 路全 NVDEC 12.6s ／ 12 路全 NVDEC 14.8s ／ 12 路 6+6 混合 11.9s
    # 差别都在几个点内——瓶颈已经不在解码并行度上了（算法服务里抠狗/向量/姿态是
    # 加锁串行的，并发再高照样排队）。所以取最简单那一格：全 NVDEC + 并发 8。
    #
    # **这个数跟算法服务的 DECODE_CPU_SHARE 是配套的，改一个必须想另一个：**
    #   share=0（默认）  → 全走 NVDEC，并发 8（两个引擎喂得饱、排队还不明显）
    #   share=0.5        → 一半走 CPU 软解，并发要提到 12，不然硬解的路反而更少
    # 只改一边是这一串里最容易踩的坑，已经踩过两次。
    #
    # 显存不是约束了：建完一路会把缓存还回去（vision_service 的 gpumem.trim），
    # 实测整卡才用 6.9/31.8 GiB。
    vision_index_concurrency: int = 8
