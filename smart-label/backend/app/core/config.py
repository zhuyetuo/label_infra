...
class Settings(BaseSettings):
    ...
    # 建画面索引时几路视频一起送。
    #
    # 2026-09-21 量过之后从 3 调到 6：建索引 93% 的时间在「解码+检测」，而那里面
    # **等解码占七成、检测只占一成半**。也就是说瓶颈不是 GPU 算力（利用率 77%
    # 时显存带宽才 3%），是每一路自己那条解码流——一路只能串行解，要快只能多开几路。
    #
    # 之前不敢往上提是怕显存；现在建完一路会把缓存还回去（vision_service 的
    # gpumem.trim），实测整卡才用 6.9/31.8 GiB，6 路的峰值在 10 GiB 上下，有余量。
    # 要是哪天换了小显存的卡，这个数往回调就是。
    vision_index_concurrency: int = 6
