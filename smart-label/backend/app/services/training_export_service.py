...
async def export_dataset(
    ...
    item_rows = await db.execute(
        select(AnnotationRecord.task_id, AnnotationLabelItem.label_id, AnnotationLabelItem.start_time_ms, AnnotationLabelItem.end_time_ms)
    ...
    for task_id, label_id, s_ms, e_ms in item_rows.all():
    ...
    for task, sample in rows:
        ...
        for label_id, s_ms, e_ms in sorted(items, key=lambda x: x[1]):
            ...
            result.append({
                ...
                "value": {
                    "start": _fmt(csv_start + timedelta(milliseconds=s_ms)),
                    "end": _fmt(csv_start + timedelta(milliseconds=e_ms)),
                    "timeserieslabels": [name_],
                },
            })
            label_counter[name_] += 1
            total_sec += (e_ms - s_ms) / 1000
