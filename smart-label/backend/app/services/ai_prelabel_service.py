...
async def _apply(
    ...
    # 模型跑了窗口、却一条片段都没给。一个小时里总有静止或活动，真实情况下几乎
    # 不可能——多半是算法服务那边的配置/后处理出了问题。**必须喊出来**：以前这种
    # 情况照样算"成功"，2026-09-23 项目 196 就是 437 个任务全空、界面上还显示
    # 「上次 AI 预标注：437 个」，看着像模型什么都没认出来
    if not inf.items and inf.n_windows > 0:
        return (f"模型跑了 {inf.n_windows} 个窗口却一条片段都没给——多半是算法服务的配置或后处理"
                f"出了问题，不是模型真没认出来（「疑似抓挠」候选照常写了）")
    return None
