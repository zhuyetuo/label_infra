...
**不是** algo_service 那张 pet_dog_daily_summary。那是另一条线：一个定时服务
从 TDengine 取项圈上报的原始 IMU，推理完写进 MySQL，后端（Java）定期去那张表
查有没有新结果——**它跟平台没有任何调用关系，只是两边都叫"日统计"**。
数据源（TDengine）、模型、后处理都不一样。两个数字放一起说会很难解释，
所以这里只管平台这一份，并且把模型版本明确标出来。
