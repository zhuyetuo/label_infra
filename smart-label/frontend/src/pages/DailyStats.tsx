 ...
 *
 * 数据是**平台自己跑的推理结果**（平台 → imu_train 的 label_service）。
 * **不是**项圈那条线的日汇总——那边是一个定时服务从 TDengine 取项圈上报的
 * 原始 IMU、推理完写 MySQL，后端定期去那张表查新结果，跟这个平台没有任何
 * 调用关系，只是两边都叫"日统计"。数据源、模型、后处理都不一样，
 * 所以这里把模型版本明确标在顶上，不跟那张表混着说。
 */
      ...
      <Alert
        ...
        description={
          <>
            ...
            <div>
              跟项圈那条线的日汇总不是同一份数据。那边是一个定时服务从 TDengine 取项圈上报的原始 IMU、
              推理完写进 MySQL，跟这个平台没有调用关系，只是两边都叫「日统计」——
              数据源、模型、后处理都不一样，两个数字别互相对着看。
            </div>
          </>
        }
      />
